...
const Profile = props => {
  ...
  const changeProfilePhoto = () => {
    ...
    axios.post('https://api.cloudinary.com/v1_1/dnnkqjrbi/image/upload', formData, {
      ...
      axios.put('/https://rio-campo-limpo.herokuapp.com/api/managers', { id: manager._id, superuser: '', image }).then(res => {
  ...
  const handleEditInfo = e => {
    ...
    validateFields(['name', 'email'], (err, values) => {
      if(!err) {
        ...
        axios.put('/https://rio-campo-limpo.herokuapp.com/api/managers', { id: manager._id, name, email, superuser: '' }).then(res => {
          setPageUpdate(!pageUpdate);
  ...
  const handleEditPassword = e => {
    ...
    validateFields(['password', 'confirmPassword'], (err, values) => {
      if(!err) {
        ...
        axios.put('/https://rio-campo-limpo.herokuapp.com/api/managers', { id: manager._id, password, superuser: '' }).then(res => {
          setPageUpdate(!pageUpdate);
          setButtonLoading(false);
          success();
        }).catch(err => {
