    ...
    validateFields(['game'], (err, values) => {
      if(!err) {
        axios.get(`/api/games/getById/${values.game}`).then(res => {
          ...
          axios.post('/https://rio-campo-limpo.herokuapp.com/api/games', { appId, name: title, link: url, image: icon, developer, score: scoreText }).then(res => {
  ...
  const handleEditGame = e => {
    ...
    validateFields(['game'], (err, values) => {
      if(!err) {
        axios.get(`/api/games/getById/${values.game}`).then(res => {
          const { title, appId, developer, url, scoreText, icon } = res.data;
          axios.put('/https://rio-campo-limpo.herokuapp.com/api/games', { id: gameModal.gameID, appId, name: title, link: url, image: icon, developer, score: scoreText }).then(res => {
            setPageUpdate(!pageUpdate);
            closeGameModal();
            success();
          }).catch(err => {
